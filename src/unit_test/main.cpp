...
static inline double uint64ToDouble(std::uint64_t value)
{
...
static inline double uint64ToDouble(std::uint64_t value)
{
    return static_cast<double>(std::int64_t(value / 2)) * 2.0 +
           static_cast<double>(std::int64_t(value & 1));
}
#endif // if !defined(JSON_USE_INT64_DOUBLE_CONVERSION)

#define JSONTEST_ASSERT_EQUAL(val1, val2) \
    ASSERT_EQ(static_cast<std::remove_reference_t<decltype(val2)>>(val1), val2)
#define JSONTEST_ASSERT_STRING_EQUAL(val1, val2) \
    ASSERT_EQ(static_cast<std::string>(val1), static_cast<std::string>(val2))
#define JSONTEST_ASSERT_PRED ASSERT_NO_FATAL_FAILURE
#define JSONTEST_ASSERT ASSERT_TRUE
#define JSONTEST_FIXTURE_LOCAL TEST_F
#define JSONTEST_ASSERT_THROWS ASSERT_ANY_THROW
...
JSONTEST_FIXTURE_LOCAL(ValueTest, integers)
{
    ...
    val = Json::Value(std::int64_t(1) << 40);
    ...
    JSONTEST_ASSERT_EQUAL((std::int64_t(1) << 40), val.asInt64());
    JSONTEST_ASSERT_EQUAL((std::int64_t(1) << 40), val.asLargestInt());
    JSONTEST_ASSERT_EQUAL((std::int64_t(1) << 40), val.asUInt64());
    JSONTEST_ASSERT_EQUAL((std::int64_t(1) << 40), val.asLargestUInt());
    JSONTEST_ASSERT_EQUAL((std::int64_t(1) << 40), val.asDouble());
    JSONTEST_ASSERT_EQUAL((std::int64_t(1) << 40), val.asFloat());
    JSONTEST_ASSERT_EQUAL(true, val.asBool());
    JSONTEST_ASSERT_STRING_EQUAL("1099511627776", val.asString());

    // 2^40 (unsigned constructor arg)
    val = Json::Value(std::uint64_t(1) << 40);
    ...
    val = Json::Value((std::int64_t(1) << 40) / 1.0);
    ...
    // int64 max
    val = Json::Value(std::int64_t(kint64max));
    ...
    JSONTEST_ASSERT_EQUAL(std::uint64_t(1) << 63, val.asUInt64());
    JSONTEST_ASSERT_EQUAL(std::uint64_t(1) << 63, val.asLargestUInt());
    JSONTEST_ASSERT_EQUAL(uint64ToDouble(std::uint64_t(1) << 63), val.asDouble());
    JSONTEST_ASSERT_EQUAL(float(std::uint64_t(1) << 63), val.asFloat());
    ...
    // int64 min
    val = Json::Value(std::int64_t(kint64min));
    ...
    // 10^19
    const auto ten_to_19 = static_cast<std::uint64_t>(1e19);
    val = Json::Value(std::uint64_t(ten_to_19));
    ...
    // uint64 max
    val = Json::Value(std::uint64_t(kuint64max));
...
JSONTEST_FIXTURE_LOCAL(ValueTest, nonIntegers)
{
    ...
    JSONTEST_ASSERT_EQUAL(-(std::int64_t(1) << 31), val.asLargestInt());
    JSONTEST_ASSERT_EQUAL(true, val.asBool());
    ...
    JSONTEST_ASSERT_EQUAL((std::int64_t(1) << 32) - 1, val.asLargestInt());
    JSONTEST_ASSERT_EQUAL((std::uint64_t(1) << 32) - std::uint64_t(1),
                          val.asLargestUInt());
    JSONTEST_ASSERT_EQUAL(true, val.asBool());
...
struct StreamWriterTest : testing::Test
{
};
...
struct CharReaderTest : testing::Test
{
};
...
JSONTEST_FIXTURE_LOCAL(CharReaderTest, parseString)
{
    ...
    }
    {
        ...
        auto charreader = b.newCharReader();
...
struct CharReaderStrictModeTest : testing::Test
{
};
...
struct CharReaderFailIfExtraTest : testing::Test
{
};
...
struct CharReaderAllowDropNullTest : testing::Test
{
...
struct CharReaderAllowNumericKeysTest : testing::Test
{
};
...
struct CharReaderAllowSingleQuotesTest : testing::Test
{
};
...
struct CharReaderAllowZeroesTest : testing::Test
{
};
...
struct CharReaderAllowSpecialFloatsTest : testing::Test
{
};
...
struct EscapeSequenceTest : testing::Test
{
};
...
JSONTEST_FIXTURE_LOCAL(EscapeSequenceTest, writeEscapeSequence)
{
    ...
    const Json::String result = Json::writeString(builder, root);
    JSONTEST_ASSERT_STRING_EQUAL(expected, result);
}

struct BuilderTest : testing::Test
{
};
...
struct RValueTest : testing::Test
{
};
...
struct MemberTemplateAs : testing::Test
{
    template <typename T, typename F>
    void EqEval(T v, F f) const
    {
        const Json::Value j = v;
        JSONTEST_ASSERT_EQUAL(j.as<T>(), f(j));
    }
};
...
JSONTEST_FIXTURE_LOCAL(MemberTemplateAs, BehavesSameAsNamedAs)
{
    ...
    JSONTEST_ASSERT_PRED(
        EqEval(Json::UInt(64), [](const Json::Value &j)
               { return j.asUInt(); }));
    JSONTEST_ASSERT_PRED(EqEval(
        std::int64_t(64), [](const Json::Value &j)
        { return j.asInt64(); }));
    JSONTEST_ASSERT_PRED(EqEval(
        std::uint64_t(64), [](const Json::Value &j)
        { return j.asUInt64(); }));
    JSONTEST_ASSERT_PRED(EqEval(Json::LargestInt(64), [](const Json::Value &j)
                                { return j.asLargestInt(); }));
    JSONTEST_ASSERT_PRED(EqEval(Json::LargestUInt(64), [](const Json::Value &j)
...
class MemberTemplateIs : public testing::Test
{
};
...
JSONTEST_FIXTURE_LOCAL(MemberTemplateIs, BehavesSameAsNamedIs)
{
    ...
    for (const Json::Value &j : values)
    {
        ...
        JSONTEST_ASSERT_EQUAL(j.is<std::int64_t>(), j.isInt64());
        JSONTEST_ASSERT_EQUAL(j.is<Json::UInt>(), j.isUInt());
        JSONTEST_ASSERT_EQUAL(j.is<std::uint64_t>(), j.isUInt64());
